...
class SettingsScreen extends StatelessWidget {
  ...
  Future<void> _backupData(BuildContext context) async {
    try {
      ...
      final file = File(
          '${dir.path}/poultry_pro_backup_${DateTime.now().millisecondsSinceEpoch}.json');

      await Share.shareXFiles([XFile(file.path)],
          text:
              'Poultry Pro Backup - ${DateTime.now().toIso8601String().split('T').first}');

      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                '✅ Backup saved and shared! Send this file to yourself on WhatsApp.'),
  ...
  Future<void> _clearAllData(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'This will permanently delete every flock, record, expense and sale.\n\nThis cannot be undone!'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('Cancel')),
    ...
    if (confirmed == true) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('All data cleared'), backgroundColor: Colors.red),
        );
      }
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Text('Keep your farm data safe. Backup weekly!',
              style: TextStyle(color: Colors.grey)),
          const SizedBox(height: 24),
          _SettingsButton(
            icon: Icons.workspace_premium,
            title: 'Upgrade to Poultry Pro',
            subtitle: 'Unlock unlimited flocks and advanced business tools',
          ...
          const Text('Built for Nigerian Poultry Farmers',
              style: TextStyle(color: Colors.grey)),
        ],
      ),
    );
